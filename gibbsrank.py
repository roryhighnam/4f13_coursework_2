...
# G - first entry is winner of game i
# M = number of players
...
def gibbs_sample(G, M, num_iters):
    ...
    for iter in tqdm(range(num_iters)):
        ...
        m = np.zeros((M, 1))
        for p in range(M):
            m[p] = np.dot(t.T, (G[:,0]==p).astype(int) - (G[:,1]==p).astype(int)) # TODO: COMPLETE THIS LINE

        iS = np.zeros((M, M))  # Container for sum of precision matrices (likelihood terms)

        # Use numpy sum to do the looping over games as much quicker than python loop
        for j in range(M):
            for k in range(j+1):
                if j==k:
                    iS[j,k] = np.sum((G[:,0]==j).astype(int) + (G[:,1]==k).astype(int))
                else:
                    iS[j,k] = -np.sum((G[:,0]==j).astype(int)*(G[:,1]==k).astype(int) + (G[:,1]==j).astype(int)*(G[:,0]==k).astype(int))
                    iS[k,j] = iS[j,k]
        # print(iS[:4,:4])
